...
const Counter = ({ end, duration = 2, suffix = "" }) => {
  ...
  return (
    <span ref={ref} className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900">
      {count}{suffix}
    </span>
  );
};
...
export default function StatsSection() {
  ...
  return (
    <div className="min-h-[40vh] bg-gradient-to-b from-white to-blue-50 py-12 px-4 sm:px-6 lg:px-8">
        ...
        >
          {stats.map((stat, idx) => (
            <motion.div key={idx} variants={cardVariants} whileHover="hover">
              <div className={`rounded-xl p-2 shadow-sm hover:shadow-lg transition border h-full ${stat.bgColor} flex flex-col items-center`}>
                ...
                <div className="text-center mb-3">
                  {isInView ? (
                    ...
                    <span className="text-4xl font-bold text-gray-300">0{stat.suffix}</span>
                  )}
                </div>
                ...
                <motion.div
                  ...
                  className="h-1.5 bg-gray-200 rounded-full w-24 mx-auto overflow-hidden mb-4"
                >
                  <motion.div
                    initial={{ x: "-100%" }}
                    whileInView={{ x: "0%" }}
                ...
                {/* Title */}
                <h3 className="text-base font-semibold text-gray-900 mb-1">{stat.title}</h3>
